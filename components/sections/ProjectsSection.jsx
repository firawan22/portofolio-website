  ...
  return (
    <section id="projects" className="relative py-20 sm:py-28 bg-dark-pure border-b border-dark-border">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 relative z-10 space-y-12 sm:space-y-16">
        {/* Header & Filter Tabs */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 sm:gap-8 pb-6 sm:pb-8 border-b border-dark-border">
          <div>
        ...
        {/* Project Cards Grid: 1 col on mobile, 2 on tablet, 3 on desktop */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {filteredProjects.map((project) => (
            ...
            >
              ...
              <div
                className={`relative h-48 sm:h-56 bg-gradient-to-br ${project.thumbnailColor} p-5 sm:p-6 flex flex-col justify-between overflow-hidden border-b border-dark-border`}
              >
                {project.image && (
                  <img
                    src={project.image}
                ...
                <div className="flex flex-wrap gap-1.5 sm:gap-2 relative z-10">
                  {project.tags.slice(0, 3).map((tag, idx) => (
                    <span
                      key={idx}
                      className="px-2 py-0.5 sm:px-2.5 sm:py-1 bg-black/60 backdrop-blur-md border border-blue-500/40 rounded-full text-[10px] sm:text-[11px] font-mono font-medium text-blue-300 shadow-sm"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div className="absolute inset-0 bg-blue-950/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3 p-4">
                  <button
                    ...
                    className="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold font-mono tracking-wider uppercase rounded-full shadow-[0_0_20px_#2563eb] flex items-center gap-2 min-h-[40px]"
                  >
              ...
              <div className="p-5 sm:p-6 space-y-4 flex-1 flex flex-col justify-between">
                ...
                <div className="pt-4 border-t border-dark-border flex items-center justify-between">
                  <button
                    onClick={() => setSelectedProject(project)}
                    className="text-xs font-mono text-blue-400 hover:text-white transition-colors flex items-center gap-1 group/link py-1"
                  >
        ...
        {selectedProject && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-dark-card border border-blue-500/50 rounded-2xl shadow-2xl overflow-hidden z-10 my-8 max-h-[90vh] flex flex-col"
            >
              {/* Modal Header */}
              <div
              ...
              <div className="p-6 sm:p-8 space-y-6 overflow-y-auto flex-1">
                ...
                {selectedProject.stats && (
                  <div className="p-4 bg-dark-surface border border-dark-border rounded-xl grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {Object.entries(selectedProject.stats).map(([k, v]) => (
                      <div key={k} className="space-y-0.5">
                        <span className="text-[10px] uppercase font-mono text-muted tracking-wider">
                          {k}
                        </span>
                        <p className="text-base sm:text-lg font-mono font-bold text-white">{v}</p>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {/* Modal Footer CTAs */}
              <div className="p-4 sm:p-6 bg-dark-surface border-t border-dark-border flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 shrink-0">
                <a
                  href={selectedProject.githubUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-5 py-2.5 bg-dark-card border border-dark-border hover:border-white text-white text-xs font-mono font-semibold rounded-lg transition-colors flex items-center justify-center gap-2 min-h-[40px]"
                >
                  <Github className="w-4 h-4" />
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-mono font-bold rounded-lg transition-all shadow-[0_0_20px_#2563eb] flex items-center justify-center gap-2 min-h-[40px]"
                >
